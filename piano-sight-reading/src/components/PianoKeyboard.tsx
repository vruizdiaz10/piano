...
export default function PianoKeyboard({ onPlayNote, highlightKey, correctKey, wrongKey, startMidi, count: countProp }: PianoKeyboardProps) {
  ...
  return (
    <div className="border border-neon-blue/20 rounded-xl overflow-hidden"
         style={{ boxShadow: '0 0 20px rgba(0,212,255,0.05), inset 0 2px 8px rgba(0,0,0,0.3)', perspective: '600px' }}>
      {isPortraitMobile && (
        <div className="flex justify-center gap-2 py-1">
          <button
            type="button"
            className="px-3 py-1 text-sm rounded bg-neon-blue/10 text-neon-cyan active:bg-neon-blue/20"
            aria-label="Octava anterior"
            onClick={() => shiftOctave(-1)}
          >
            &#9664;
          </button>
          <span className="text-xs text-neon-blue/50 self-center">
      ...
      <div ref={containerRef} className="py-1.5 select-none">
        <div className="flex relative h-36 lg:h-[100px] mx-auto" style={{ width: containerWidth, transformStyle: 'preserve-3d', transform: 'rotateX(4deg)' }} role="group" aria-label="Teclado de piano">
        {whiteKeys.map(midi => {
          ...
          return (
            <button
              ...
              type="button"
              className={cn(
                'h-36 lg:h-[100px] border border-neon-blue/10 rounded-b-[4px] bg-gradient-to-b from-white to-[var(--ivory)] cursor-pointer flex flex-col justify-end items-center pb-1 text-xs text-neon-blue/40 transition-colors duration-100 hover:to-neon-cyan/5 active:bg-neon-cyan/10 key-press',
                isHighlighted && '!bg-gradient-to-b !from-neon-cyan/40 !to-neon-blue/20 !text-neon-cyan',
                isCorrect && '!bg-neon-green/30 !text-neon-green',
                isWrong && '!bg-neon-pink/20 !text-neon-pink',
          ...
          return (
            ...
            >
              <button
                type="button"
                className={cn(
                  'border border-black/80 rounded-b-[4px] bg-gradient-to-b from-[var(--ebony)] to-black cursor-pointer transition-colors duration-100 hover:bg-white/10 key-press-black',
                  isHighlighted && '!bg-neon-cyan/40',
                  isCorrect && '!bg-neon-green',
                  isWrong && '!bg-neon-pink/40',
                  isCorrect && 'key-sparkle relative'
                )}
